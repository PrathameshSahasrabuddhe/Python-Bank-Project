...
class Bank:
    ...
    def DeleteAccount(name:str, acc_no:str, balance:int, transaction_list:list):
        if ImpProgs.FileCheck(acc_no):
            print("File Found!")
            ch = str(input("Are you sure you want to delete your account?(Yes/No): "))
            if ch.casefold() == "yes":
                complete = ImpProgs.FullPath(acc_no)
                os.remove(complete)
                print("Account Deleted Successfully!") ### Make sure that file doesn't exist ###
                Bank.EntryOptions()
            else:
    ...
    def BankBasedOptions(name:str, acc_no:str, balance:int, trasaction_list:list):
        while True:
            ...
            if choice == 1:
                ...
                Bank.DeleteAccount(name, acc_no, balance, trasaction_list)
            elif choice == 5:
                print("\n\n")
                Bank.EntryOptions()
